...
spark = (
    ...
    .appName("kafka-to-cassandra-mysql-stream")
...
def process_batch(batch_df, batch_id):
    ...
    # 2) Aggregate theo giờ rồi ghi MySQL
    agg_df = (
        prepared_df
        ...
        .withColumn(
            "spend_component",
            F.when(
                F.col("custom_track") == "click",
                F.coalesce(F.col("bid"), F.lit(0.0))
            ).otherwise(F.lit(0.0))
        )
...
query = (
    ...
    .outputMode("append")
    .option("checkpointLocation", "/tmp/checkpoints/kafka_to_cassandra_mysql_stream")
